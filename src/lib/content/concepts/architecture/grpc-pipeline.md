...
- **Thinking on or off**: `thinking.type` (`enabled` or `disabled`) sets the preference. Without it, a `reasoning_effort` of `"none"` or `"minimal"` means off — except for templates that toggle thinking through their own `reasoning_effort` on/off words (Hy4-style, `high`/`no_think`): there only the template's off word (`no_think`) turns thinking off, and any other effort value, `none` and `minimal` included, turns it on (the template default) and overrides `thinking.type`. SMG writes the preference under the key the template actually reads: `enable_thinking` (Qwen3, GLM, Nemotron), `thinking` (DeepSeek, Kimi), `thinking_mode` set to `"enabled"` or `"disabled"` (MiniMax-M3), or `reasoning_effort` set to the template's own on or off word (Hy4-style). A value you pass for that key in `chat_template_kwargs` wins.
