...
!!! warning "History backends in chart 1.11.0"
    With `history.backend` set to `postgres` or `redis`, or to `oracle` with `history.oracle.dsn`, chart 1.11.0 passes a flag the gateway image rejects (`--postgres-pool-max-size`, `--redis-pool-max-size`, or `--oracle-dsn`), and the router exits at startup. Chart versions after 1.11.0 pass the flag names the gateway image accepts (`--postgres-pool-max`, `--redis-pool-max`, `--oracle-connect-descriptor`, and `--oracle-username`), so these `history.backend` settings work there. On chart 1.11.0, leave `history.backend` at `memory` and pass the history flags through `router.extraArgs` instead, for example `--history-backend postgres --postgres-db-url <url>`.
